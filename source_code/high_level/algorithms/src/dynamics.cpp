...
Direction_Vector Dynamics::computeZeroMomentPoint(const Direction_Vector &accel, const Direction_Vector &gyro, const Direction_Vector &gyro_dot)
{
    ...
    Eigen::Vector3f v_CoM_to_IMU = {-CoM_world_frame.x, -CoM_world_frame.y, -CoM_world_frame.z}; // Vector from CoM to IMU in world frame

    // CoM acceleration in world frame calculated using rigid body dynamics: a_B = a_A + w_dot x (r_B - r_A) + w x (w x (r_B - r_A))
    Eigen::Vector3f v_CoM_accel_world_frame = v_IMU_accel + v_IMU_gyro_dot.cross(v_CoM_to_IMU) + v_IMU_gyro.cross(v_IMU_gyro.cross(v_CoM_to_IMU));
